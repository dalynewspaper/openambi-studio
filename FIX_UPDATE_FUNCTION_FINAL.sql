-- FIX UPDATE FUNCTION - FINAL VERSION
-- The function can't find recordings even though they exist
-- This version adds better debugging and ensures the query works
...
BEGIN
    ...
    -- Ensure p_id is a valid UUID
    v_recording_id := p_id;
    
    -- First check if recording exists at all
    -- Use explicit UUID comparison - this should work with SECURITY DEFINER
    ...
    IF v_recording_user_id IS NULL THEN
        -- Try to find it with text comparison to see if it's a format issue
        SELECT user_id, deleted_at 
        INTO v_recording_user_id, v_deleted_at
        FROM user_recordings
        WHERE id::text = v_recording_id::text;
